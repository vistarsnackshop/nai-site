//this api is for fetching the data for the inventory stats graph(prior 4 weeks) and current week sales
...
async function fetchData(itemId:string, whsId:string) {
  ...
  try {
    ...
    const inventoryData = await db.query("select ORDBSQT, FSCLYRNB, FSCLWKNB from nai.SALESHST where WHSID = ? and ITMID = ?", [whsId, itemId]);
    await db.close();
